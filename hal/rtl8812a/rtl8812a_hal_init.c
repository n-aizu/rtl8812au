	...
	)
{
	...
	if (!AutoLoadFail)
	{
		tempval = hwinfo[EEPROM_RF_BOARD_OPTION_8812];
		if( ((tempval & 0xe0)>>5) == 0x1)// [7:5]
			pHalData->EEPROMBluetoothCoexist = _TRUE;
		else
			pHalData->EEPROMBluetoothCoexist = _FALSE;

		tempval = hwinfo[EEPROM_RF_BT_SETTING_8812];
...
VOID
Hal_EfuseParseBTCoexistInfo8821A(
	IN PADAPTER			Adapter,
	IN u8				*hwinfo,
	IN BOOLEAN			AutoLoadFail
	)
{
	...
	if(!AutoLoadFail)
	{
		tmpu4 = rtw_read32(Adapter, REG_MULTI_FUNC_CTRL);
		if(tmpu4 & BT_FUNC_EN)
		...
		tempval = hwinfo[EEPROM_RF_BT_SETTING_8821];
		pHalData->EEPROMBluetoothAntNum = (tempval&0x1);					// bit [0]

		if(IS_HARDWARE_TYPE_8821S(Adapter))
		{	// For windows driver, 8821S does not enable BTCoexist
			pHalData->EEPROMBluetoothCoexist = 0;
		}
	}
	else
...
#ifdef CONFIG_BT_COEXIST
	hal_btcoex_SetBTCoexist(Adapter, pHalData->EEPROMBluetoothCoexist);
	hal_btcoex_SetChipType(Adapter, pHalData->EEPROMBluetoothType);
	hal_btcoex_SetPgAntNum(Adapter, pHalData->EEPROMBluetoothAntNum);
#endif
}
		...
		case HW_VAR_CHECK_TXBUF:
			{
				...
				{
					init_reg_200 = reg_200 = rtw_read32(padapter, 0x200);
					init_reg_204 = reg_204 = rtw_read32(padapter, 0x204);
					i++;
					if (reg_200 != reg_204) {
				...
				if (pass_ms >= 2000 || reg_200 != reg_204)
				{
					DBG_8192C("%s: (HW_VAR_CHECK_TXBUF)TXBUF not Empty(%d) in %d ms\n", __FUNCTION__, i, pass_ms);
					DBG_8192C("%s: (HW_VAR_CHECK_TXBUF)0x200=0x%08x, 0x204=0x%08x (0x%08x, 0x%08x)\n",
						__FUNCTION__, reg_200, reg_204, init_reg_200, init_reg_204);
					//rtw_warn_on(1);
				} else {
					DBG_8192C("%s: (HW_VAR_CHECK_TXBUF)TXBUF Empty(%d) in %d ms\n", __FUNCTION__, i, pass_ms);
				}
